...
def draw(ax=None): #TODO: TEST
        ...
        for element in Element.elements.values():
            ax.plot([element.nodes[0].coordinates[0], element.nodes[1].coordinates[0]], 
                    [element.nodes[0].coordinates[1], element.nodes[1].coordinates[1]], 'k-')
        ...
        for node in Node.nodes.values():
            ...
            # Draw constraints
            if np.any(node.BCs):
                if node.BCs[0]:  # constraint in x direction
                ...
                if node.BCs[1]:  # constraint in y direction
                    triangle = plt.Polygon([
                        [node.coordinates[0], node.coordinates[1]], 
